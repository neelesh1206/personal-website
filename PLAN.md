...
| Interview Prep    | `/admin/interview-prep`  | ✅     | Recruiter screen Q&A reference. Pinned delivery rules + accordion of cues/answer/follow-ups with markdown rendering of \*(pause)\* markers. Sticky search. Inline edit + JSON import (merge/replace). DB-backed (prep_interview_questions in Neon) — never committed to the public repo.                                                                                                                                                                                                                                                                                                                                                                                     |
| Coding Prep       | `/admin/coding-prep`     | ✅     | Honest-scoreboard / flexible-plan interview-prep tracker — 4 tabs (Today routine with sticky top bar, dominant next-action, done-row collapse, Pomodoro ring; 15-Day Plan with re-solve-from-blank toggle; Reference Library with **Read + Flashcards** modes (SM-2-lite, hint+reveal, Got it / Almost / Missed grading, mobile-first); Dashboard with streaks/heatmap/XP-time/per-week/mood charts + 14 medallion badges (incl. Active Recall) + Track 2 stub). XP ledger (Apprentice→Practitioner→Senior→Staff). Plan-day slide + load modes. Optional HF re-plan. **Two crons**: morning brief (14:30 UTC) + graded evening summary (04:00 UTC). 12 Neon prep\_\* tables. |
