...
| Coding Prep       | `/admin/coding-prep`     | ✅     | Honest-scoreboard / flexible-plan interview-prep tracker — 4 tabs (Today routine with sticky top bar, dominant next-action, done-row collapse, Pomodoro ring; 10-Day Plan with re-solve-from-blank toggle; Reference Library; Dashboard with streaks/heatmap/XP-time/per-week/mood charts + 13 medallion badges + Track 2 stub). XP ledger (Apprentice→Practitioner→Senior→Staff). Plan-day slide + load modes (full/core/re-entry/maintenance). Optional HF re-plan, graded daily email/Slack tone tiers w/ emoji bullets via Vercel Cron. 11 Neon prep\_\* tables. |
